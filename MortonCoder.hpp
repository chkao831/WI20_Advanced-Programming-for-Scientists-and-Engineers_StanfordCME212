 ...
 * @ref https://www.forceflow.be/2013/10/07/morton-encodingdecoding-through-bit-interleaving-implementations/
 */
inline uint32_t spread_bits(uint32_t x) {
    ...
    for(unsigned int k = 1; k <= total_bits; k++){
        // 1 is in the (k+1)th rightmost position
        uint32_t one = 1 << k;
        // only looking at the last few digits
        uint32_t last = x & one;
        // Shifted to the left
        uint32_t shifted = last << 2*k;
...
/* My Version II that performs same thing */
//inline uint32_t spread_bits(uint32_t x) {
//    x &= 0x000003ff; //turn off bits off except for last 10
//    x = (x ^ (x << 16)) & 0xff0000ff; //(self or shifting 16 bits) & mask
//    x = (x ^ (x <<  8)) & 0x0300f00f;
//    x = (x ^ (x <<  4)) & 0x030c30c3;
//    x = (x ^ (x <<  2)) & 0x09249249;
//  return x;
...
inline uint32_t compact_bits(uint32_t x) {
    ...
    for(unsigned int k = 1; k <= total_bits; k++){
        // Starting from k=0,3,6,...3k from the right
        uint32_t one = 1 << 3*k;
        // only looking at the last few digits
        uint32_t last = x & one;
